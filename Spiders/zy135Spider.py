    ...
    def getMovieInfoItems(self, movieUrls):
        ...
        for i, infourl in enumerate(movieUrls):
            try:
                ...
                item.mAlias = html.xpath('//div[@class="vodinfobox"]/ul//li[2]//span')[0].text + ' '
                ...
                item.mUpdate = html.xpath('//div[@class="vodinfobox"]/ul//li[8]//span')[0].text + ' '
                item.mCover = html.xpath('//img[@class="lazy"]/@src')[0]
                item.mSumary = html.xpath('//*[@class="vodplayinfo"][1]//text()')[0] + ' '
                # print(item.mSumary)
    ...
    def save2mysql(self, items):
        ...
        try:
            with connection.cursor() as cursor:
                # Create a new record
                sql = "INSERT INTO `movie` (`mId`, `mName`, `mAlias`, `mCover`, `mScore`,`mDirector`,`mStar`,`mType`,`mArea`,`mYear`,`mUpdate`,`mSumary`,`mPlayUrl_m3u8`,`mPlayUrl_mp4`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
                for item in items:
                    try:
                        mId = item.mId
                        mName = item.mName.encode('utf-8')
                        mAlias = item.mAlias.encode('utf-8')
                        ...
                        mUpdate = item.mUpdate.encode('utf-8')
                        ...
                        cursor.execute(sql, (mId, mName,mAlias,mCover,mScore,mDirector,mStar,mType,mArea,mYear,mUpdate,mSumary,mPlayUrl_mp4,mPlayUrl_m3u8))
                    except Exception as e:
                        print('写入失败：'+mName)
                        continue
...
# Arg1: 指定爬取的页数
# Arg2: 指定爬取影片类型
# [电影，连续剧，综艺，动漫，伦理片，美女热舞，VIP视频秀，纪录片，街拍美女]
Spider = zySpider(3, '电影')
