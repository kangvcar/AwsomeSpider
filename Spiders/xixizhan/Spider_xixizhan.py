...
class getMovieInfo(object):
	...
	def __init__(self, url):
		...
		# print u'写入数据库成功'
		self.pipeline2excel = self.pipeline2excel(self.items)
		print u'写入excel成功'
	...
	def pipeline2excel(self, items):
		w = Workbook()
		ws = w.add_sheet(u'欧美最新电影')
		for row, item in enumerate(items):
			col = 0
			ws.write(row, col, item.mtype)
			col += 1
			ws.write(row, col, item.mtitle)
			col += 1
			ws.write(row, col, item.mauthor)
			col += 1
			ws.write(row, col, item.mtime)
			col += 1
			ws.write(row, col, str(item.downloadlink))
			col += 1
		w.save(u'xixizhan.xls')
